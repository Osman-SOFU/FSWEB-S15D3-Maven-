...
public class WordCounter {
    ...
    public static Map<String,Integer> calculatedWord(){
        ...
        // Kelimeleri ayrıştırmak için regex kullanarak metni temizleyip küçük harfe çeviriyoruz.
        // Tüm noktalama işaretlerini ve özel karakterleri kaldırıyoruz.
        String cleanedText = text.replaceAll("[^a-zA-Z0-9’'\\-\\s]", "").toLowerCase();

        // Boşluklara göre bölüyoruz
        String[] words = cleanedText.split("\\s+");

        Map<String, Integer> wordCountMap = new HashMap<>();

        // Her kelimeyi sayarak map'e ekliyoruz
        for (String word : words) {
            if (word.isEmpty()) continue; // boş string kontrolü

            // Kelime haritada varsa sayısını artır, yoksa 1 olarak ekle
